...
## Test
"test" folder contains a test coded in [Go](https://golang.org/) language. In order to run it, you must install Go and run `go run .\main.go .\utils.go [-v]` command in a terminal or `go build` and then `test.exe [-v]`. Optional argument `-v` will show a detailed output of the tests.
