...
typedef error_tc (*HashAlgoCompute)(const void *data, size_t length,
   uint8_t *digest);
...
typedef error_tc (*CipherAlgoInit)(void *context, const uint8_t *key,
   size_t keyLen);
...
typedef error_tc (*KemAlgoGenerateKeyPair)(const PrngAlgo *prngAlgo,
   void *prngContext, uint8_t *pk, uint8_t *sk);

typedef error_tc (*KemAlgoEncapsulate)(const PrngAlgo *prngAlgo,
   void *prngContext, uint8_t *ct, uint8_t *ss, const uint8_t *pk);

typedef error_tc (*KemAlgoDecapsulate)(uint8_t *ss, const uint8_t *ct,
   const uint8_t *sk);

//Common API for pseudo-random number generators (PRNG)
typedef error_tc (*PrngAlgoInit)(void *context);

typedef error_tc (*PrngAlgoSeed)(void *context, const uint8_t *input,
   size_t length);

typedef error_tc (*PrngAlgoAddEntropy)(void *context, uint_t source,
   const uint8_t *input, size_t length, size_t entropy);

typedef error_tc (*PrngAlgoRead)(void *context, uint8_t *output, size_t length);
