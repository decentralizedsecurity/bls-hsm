...
error_tc ecdsaWriteSignature(const EcdsaSignature *signature, uint8_t *data,
   size_t *length)
{
   error_tc error;
   size_t k;
...
error_tc ecdsaReadSignature(const uint8_t *data, size_t length,
...
__weak_func error_tc ecdsaGenerateSignature(const PrngAlgo *prngAlgo, void *prngContext,
...
__weak_func error_tc ecdsaVerifySignature(const EcDomainParameters *params,
