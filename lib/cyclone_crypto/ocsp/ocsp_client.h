...
typedef error_tc (*OcspClientTlsInitCallback)(HttpClientContext *context,
...
error_tc ocspClientInit(OcspClientContext *context);

#if (OCSP_CLIENT_TLS_SUPPORT == ENABLED)

error_tc ocspClientRegisterTlsInitCallback(OcspClientContext *context,
   OcspClientTlsInitCallback callback);

#endif

error_tc ocspClientSetPrng(OcspClientContext *context, const PrngAlgo *prngAlgo,
   void *prngContext);

error_tc ocspClientSetTimeout(OcspClientContext *context, systime_t timeout);

error_tc ocspClientSetHost(OcspClientContext *context, const char_t *host);
error_tc ocspClientSetUri(OcspClientContext *context, const char_t *uri);

error_tc ocspClientBindToInterface(OcspClientContext *context,
   NetInterface *interface);

error_tc ocspClientConnect(OcspClientContext *context,
   const IpAddr *serverIpAddr, uint16_t serverPort);

error_tc ocspClientCreateRequest(OcspClientContext *context,
   const char_t *cert, size_t certLen, const char_t *issuerCert,
   size_t issuerCertLen);

error_tc ocspClientSendRequest(OcspClientContext *context);

error_tc ocspClientValidateResponse(OcspClientContext *context,
   const char_t *cert, size_t certLen, const char_t *issuerCert,
   size_t issuerCertLen);
...
error_tc ocspClientDisconnect(OcspClientContext *context);
error_tc ocspClientClose(OcspClientContext *context);
