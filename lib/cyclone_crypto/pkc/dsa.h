...
error_tc dsaWriteSignature(const DsaSignature *signature, uint8_t *data, size_t *length);
error_tc dsaReadSignature(const uint8_t *data, size_t length, DsaSignature *signature);

error_tc dsaGenerateSignature(const PrngAlgo *prngAlgo, void *prngContext,
   const DsaPrivateKey *key, const uint8_t *digest, size_t digestLen,
   DsaSignature *signature);

error_tc dsaVerifySignature(const DsaPublicKey *key,
