...
error_tc x509ValidateCertificate(const X509CertInfo *certInfo,
...
error_tc x509CheckSubjectName(const X509CertInfo *certInfo,
   const char_t *fqdn)
{
   error_tc error;
   bool_t res;
...
error_tc x509CheckNameConstraints(const char_t *subjectName,
...
error_tc x509ParseIpv4Addr(const char_t *str, uint8_t *ipAddr)
{
   error_tc error;
   int_t i = 0;
   int_t value = -1;
...
error_tc x509ParseIpv6Addr(const char_t *str, uint8_t *ipAddr)
{
   error_tc error;
   int_t i = 0;
   int_t j = -1;
