...
function functionalInterfaceFactory({ SecretKey, PublicKey, Signature, }) {
    /**
    ...
    function sign(secretKey, message) {
        helpers_1.validateBytes(secretKey, "secretKey");
        helpers_1.validateBytes(message, "message");
    ...
    function verify(publicKey, message, signature) {
        helpers_1.validateBytes(publicKey, "publicKey");
        helpers_1.validateBytes(message, "message");
        helpers_1.validateBytes(signature, "signature");
        try {
            return Signature.fromBytes(signature).verify(PublicKey.fromBytes(publicKey), message);
        }
        catch (e) {
            if (e instanceof errors_1.NotInitializedError)
                throw e;
            return false;
        }
    }
    ...
    function verifyAggregate(publicKeys, message, signature) {
        helpers_1.validateBytes(publicKeys, "publicKey");
        helpers_1.validateBytes(message, "message");
        helpers_1.validateBytes(signature, "signature");
        try {
    ...
    function verifyMultiple(publicKeys, messages, signature) {
        helpers_1.validateBytes(publicKeys, "publicKey");
        helpers_1.validateBytes(messages, "message");
        helpers_1.validateBytes(signature, "signature");
     ...
     */
    function verifyMultipleSignatures(publicKeys, messages, signatures) {
        helpers_1.validateBytes(publicKeys, "publicKey");
        helpers_1.validateBytes(messages, "message");
        helpers_1.validateBytes(signatures, "signatures");
        if (publicKeys.length === 0 || publicKeys.length !== messages.length || publicKeys.length !== signatures.length) {
            return false;
        }
        try {
            return Signature.verifyMultipleSignatures(publicKeys.map((publicKey) => PublicKey.fromBytes(publicKey)), messages.map((msg) => msg), signatures.map((signature) => Signature.fromBytes(signature)));
        }
        catch (e) {
            if (e instanceof errors_1.NotInitializedError)
                throw e;
...
exports.functionalInterfaceFactory = functionalInterfaceFactory;
